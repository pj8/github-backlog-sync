...
async function run() {
  try {
    const backlogSpaceName = core.getInput('BACKLOG_SPACE_NAME');
    const backlogSpaceDomain = core.getInput('BACKLOG_SPACE_DOMAIN');
    const backlogApiKey = core.getInput('BACKLOG_API_KEY');
    const backlogProjectKey = core.getInput('BACKLOG_PROJECT_KEY');
    ...
    for (const backlogIssueMatch of backlogIssueMatches) {
      ...
      try {
        ...
        if (error.code === 'ECONNABORTED') {
          core.setFailed(`リクエストがタイムアウトしました: ${error}`);
        } else {
          core.setFailed(`Backlog課題の更新に失敗しました: ${error}`);
        }
      }
    }
  } catch (error) {
